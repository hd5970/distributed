...
func doMap(
	...
	if err != nil {
		log.Fatal("Fail to open file while doing map: ", err.Error())
	}
	//fi, err := file.Stat()
	//if err != nil {
	...
	if err != nil {
		log.Fatal("Fail to stat file while doing map: ", err.Error())
	}
	bufferBytes := make([]byte, fileInfo.Size())
	_, err = file.Read(bufferBytes)
	if err != nil {
		log.Fatal("Fail to read file while doing map: ", err.Error())
	}
	file.Close()
	KvList := mapF(inFile, string(bufferBytes))
	...
	//
	// todo I'm gonna do this by myself
	for r := 0; r < nReduce; r++ {
		file, err := os.Create(reduceName(jobName, mapTaskNumber, r))
		if err != nil {
			log.Fatal("Fail to create file while doing map:", err.Error())
		}
		enc := json.NewEncoder(file)
		for _, kv := range KvList {
			if ihash(kv.Key)%nReduce == r {
				err = enc.Encode(&kv)
				if err != nil {
					log.Fatal("Fail to encode json kv pair:", err.Error())
				}
			}
		}
		file.Close()
